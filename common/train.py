...
_stop_words: set = set()
_tokeniser: TweetTokenizer = None

def _init_worker(sw):
    global _stop_words, _tokeniser
    _stop_words = sw
    _tokeniser = TweetTokenizer(strip_handles=True, reduce_len=True)

def _tokenise_row(text):
    if not text:
        return []
    tokens = _tokeniser.tokenize(text)
    return [w for w in (_PUNCT.sub('', t) for t in tokens) if w and w not in _stop_words]
...
def train(csv_folder: str = None):
    ...
    try:
        with tqdm(total=len(stages), desc="Training pipeline", unit="stage",
            ...
            with Pool(n_jobs, initializer=_init_worker, initargs=(stop_words,)) as pool:
                results = list(tqdm(
                    pool.imap(_tokenise_row, df['Mutated Text Content'], chunksize=2000),
                    total=len(df), desc="  Tokenising", position=1, leave=False
                ))
            ...
            tfidf = TfidfVectorizer(ngram_range=(1, 2), max_features=5000, analyzer=lambda x: x)
